// Standalone, zero-dependency Bigin submission server for the counseling form.
//
...
if (!CLIENT_ID || !CLIENT_SECRET || !REFRESH_TOKEN) {
  console.error(
    "[counseling] Missing ZOHO_CLIENT_ID / ZOHO_CLIENT_SECRET / ZOHO_REFRESH_TOKEN in .env"
  );
  process.exit(1);
}
...
async function forwardLead(body, biginId, source = "counseling-form") {
  ...
  try {
    ...
    console.log(`[counseling] leads API forward → ${res.status}`);
  } catch (err) {
    console.error("[counseling] leads API forward failed:", err?.message || err);
  }
}
    ...
    req.on("end", async () => {
      try {
        ...
        if (body.company && String(body.company).trim()) {
          console.warn(`[counseling] honeypot triggered: ${ip}`);
          return sendJson(res, 200, { ok: true });
        }
        ...
        console.error("[counseling] error:", err?.message || err, err?.detail || "");
